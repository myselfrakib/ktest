  ...
  try {
    ...
    const profileResponse = await axios.get(
      `https://graph.instagram.com/v21.0/me`,
      {
        params: {
          fields:
            "id,username,account_type,media_count,followers_count,profile_picture_url,name",
    ...
    updates.push(
      usersRef.set({ instagram: instagramData, isInstagramConnected: true }, {
        merge: true,
      }),
    )
...
export const instagramDeauthorize = functions.https.onRequest(
  async (req, res) => {
    if (req.method !== "POST") {
    ...
    try {
      ...
      const creatorsSnap = await db
        .collection("creators")
        .where("instagram.igUserId", "==", igUserId)
        .get()
...
export const instagramDeleteData = functions.https.onRequest(
  async (req, res) => {
    if (req.method !== "POST") {
      res.status(405).send("Method Not Allowed")
      return
